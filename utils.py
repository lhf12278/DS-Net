...
def validation(net, val_data_loader, device, save_tag=False):
    ...
    for batch_id, val_data in enumerate(val_data_loader):
        with torch.no_grad():
            ...
            haze = haze.to(device)
            gt = gt.to(device)
            SobelClear, res_dehaze, res_sobel = net(haze, gt)

        # --- Calculate the average PSNR --- #
        psnr_list.extend(to_psnr(res_dehaze, gt))

        # --- Calculate the average SSIM --- #
        ssim_list.extend(to_ssim_skimage(res_dehaze, gt))

        # --- Save image --- #
        if save_tag:
            save_image(res_dehaze, image_name)
